...
export class Puzzle extends Entity {
    ...
    this.preItemCheckId = this.player.addPreItemCheck(() => {
      ...
      const closestRow =
        rows.reduce((prev, curr) => {
          return Math.abs(curr - this.player.y) < Math.abs(prev - this.player.y)
            ? curr
            : prev;
        }) -
        PUZZLE_SIZE / 2;
      ...
      const closestColumn =
        columns.reduce((prev, curr) => {
          return Math.abs(curr - this.player.x) < Math.abs(prev - this.player.x)
            ? curr
            : prev;
        }) -
        PUZZLE_SIZE / 2;
      ...
      const puzzlePart = this.player.inhand ? this.player.inhand.entity : null;
      ...
      if (puzzlePart && puzzlePart instanceof PuzzlePart) {
  ...
  render(ctx) {
    ...
    ctx.lineTo(this.x + this.width - ctx.lineWidth / 2, this.y + this.height);

    ctx.moveTo(this.x + this.width, this.y + this.height - ctx.lineWidth / 2);
